...
function getNavItems(role: UserRole) {
  switch (role) {
    case UserRole.SUPER_ADMIN: return superAdminNavItems;
    case UserRole.TEACHER: return teacherNavItems;
    case UserRole.ACCOUNTANT: return accountantNavItems;
...
function getRoleBadge(role: UserRole) {
  switch (role) {
    case UserRole.SUPER_ADMIN: return { label: "Super Admin", className: "bg-violet-100 text-violet-700 dark:bg-violet-900/50 dark:text-violet-300" };
    case UserRole.SCHOOL_ADMIN: return { label: "Admin", className: "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300" };
    case UserRole.TEACHER: return { label: "Teacher", className: "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300" };
    case UserRole.ACCOUNTANT: return { label: "Accounts", className: "bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300" };
  }
}

export function Sidebar({ userRole, schoolName }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className={cn("flex items-center h-14 px-4 border-b gap-3", collapsed && "justify-center")}>
      ...
      {!collapsed && schoolName && (
        <div className="px-3 py-2.5 border-b">
          <div className="bg-muted/50 rounded-lg px-3 py-2">
            ...
            <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full", roleBadge.className)}>
              {roleBadge.label}
            </span>
          </div>
        </div>
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn("w-4 h-4 flex-shrink-0", isActive ? "text-violet-600 dark:text-violet-400" : "group-hover:text-foreground")} />
              <AnimatePresence>
                {!collapsed && (
                  <motion.span
                    initial={{ opacity: 0 }}
