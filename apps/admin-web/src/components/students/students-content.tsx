...
export function StudentsContent({
  ...
  const handleViewReportCard = async (examId: string) => {
    if (!detailsModal.student?.id) return;
    setReportCardModal({ open: true, loading: true, data: null });
    try {
      const card = await getStudentReportCard(examId, detailsModal.student.id);
      setReportCardModal({ open: true, loading: false, data: card });
    } catch (err: any) {
  ...
  const filteredAndSorted = useMemo(() => {
    let result = students.filter((s) => {
      ...
      const matchFeeStatus =
        ...
        (selectedFeeStatus === "NO_DUES"
          ? studentFeeStatus === "NO DUES" || studentFeeStatus === "PAID"
          : studentFeeStatus === selectedFeeStatus);

      // Attendance Status matching
      const studentAttendanceStatus =
        s.attendances?.[0]?.status ?? "NOT_MARKED";
    ...
    result = [...result].sort((a, b) => {
      switch (sortBy) {
        ...
        case "roll_desc": {
          ...
          return b.rollNumber.localeCompare(a.rollNumber, undefined, {
            numeric: true,
          });
        }
        case "name_asc":
        ...
        case "admission_asc":
          return a.admissionNumber.localeCompare(b.admissionNumber, undefined, {
            numeric: true,
          });
        case "admission_desc":
          return b.admissionNumber.localeCompare(a.admissionNumber, undefined, {
            numeric: true,
          });
        default:
          return 0;
  ...
  return (
    ...
    >
      <div className="p-6 space-y-5 max-w-[1400px]">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {paginated.map((s, i) => {
                ...
                const attConfig =
                  ATTENDANCE_BADGE[attRecord] ?? ATTENDANCE_BADGE.NOT_MARKED;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="h-12 px-4 text-xs">
                    ...
                    <td className="h-12 px-4">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border ${attConfig.badge}`}
                      >
                        {attConfig.label}
                      </span>
                    </td>
                    <td className="h-12 px-4">
                      {(() => {
                        ...
                        return (
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border ${FEE_BADGE[feeStatus] ?? FEE_BADGE.PAID}`}
                          >
                            {feeStatus}
                          </span>
                        );
                    ...
                    <div className="flex items-center justify-between pr-8">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground font-normal">
                          {detailsModal.student.class?.name} ·{" "}
                          {detailsModal.student.section?.name} | Roll No:{" "}
                          {detailsModal.student.rollNumber}
                        </p>
                      </div>
                      {outstanding > 0 &&
                        canEdit &&
                        detailsModal.activeTab !== "2" && (
                          <button
                            type="button"
                            onClick={() =>
            ...
            (() => {
              ...
              return (
                <Tabs
                  ...
                  onChange={(key) =>
                    setDetailsModal((prev) => ({ ...prev, activeTab: key }))
                  }
                  items={[
                    {
                      key: "1",
                        ...
                        <div className="grid grid-cols-2 gap-4 py-4 max-h-[60vh] overflow-y-auto scrollbar-hide">
                          <div>
                            ...
                            <p className="text-sm font-medium">
                              {detailsModal.student.admissionNumber}
                            </p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground font-medium uppercase mb-1">
                          ...
                          <div className="col-span-2 pt-4 border-t mt-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <p className="text-sm font-medium">
                                  {detailsModal.student.parentName}
                                </p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground font-medium uppercase mb-1">
                                ...
                                <p className="text-sm font-medium">
                                  {detailsModal.student.parentPhone}
                                </p>
                              </div>
                              <div className="col-span-2">
                                ...
                                <p className="text-sm font-medium">
                                  {detailsModal.student.address || "N/A"}
                                </p>
                              </div>
                            </div>
                          </div>
                      ...
                      children: (
                        <div className="max-h-[65vh] overflow-y-auto scrollbar-hide pr-1">
                          <StudentFeeCollection
                            ...
                            canEdit={canEdit}
                            onPaymentSuccess={() =>
                              refreshStudentDetails(detailsModal.student.id)
                            }
                            onWaiveSuccess={() =>
                              refreshStudentDetails(detailsModal.student.id)
                            }
                    ...
                    },
                    {
                      ...
                      children: (
                        <div className="py-4 space-y-4 max-h-[60vh] overflow-y-auto scrollbar-hide">
                          ...
                          {detailsModal.student.advanceLedgers &&
                            detailsModal.student.advanceLedgers.length > 0 && (
                              <div className="p-4 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg border border-emerald-100 dark:border-emerald-900 mb-6">
                                ...
                                <div className="space-y-2">
                                  {detailsModal.student.advanceLedgers.map(
                                    (ledger: any) => (
                                      <div
                                        key={ledger.id}
                                      ...
                                      >
                                        <span className="text-emerald-700 dark:text-emerald-400">
                                          {ledger.description || "Advance Paid"}
                                        </span>
                                        <span className="font-semibold text-emerald-800 dark:text-emerald-300">
                                          {formatCurrency(
                                            parseFloat(ledger.amount),
                                          )}
                                        </span>
                                      </div>
                                    ),
                          ...
                          {detailsModal.student.feeCharges &&
                          detailsModal.student.feeCharges.length > 0 ? (
                            <div className="space-y-3">
                              {detailsModal.student.feeCharges.map(
                                (charge: any) => {
                                  const totalAmount =
                                    charge.items?.reduce(
                                      (sum: number, item: any) =>
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        <div className="text-right">
                                          ...
                                          <span
                                            className={`inline-flex items-center px-2 py-0.5 mt-1 rounded-full text-[10px] font-medium border ${FEE_BADGE[charge.status] ?? FEE_BADGE.PAID}`}
                                          >
                                            {charge.status}
                                          </span>
                                        </div>
                                                  ...
                                                  (item: any) => {
                                                    ...
                                                    return (
                                                      ...
                                                      >
                                                        ...
                                                        <td
                                                          className={`py-2 text-right font-semibold ${due > 0 ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400"}`}
                                                        >
                                                          {formatCurrency(due)}
                                                        </td>
                                                      </tr>
                          ...
                          {detailsModal.student.paymentTxs &&
                          detailsModal.student.paymentTxs.length > 0 ? (
                            <div className="space-y-3 mt-3">
                              {detailsModal.student.paymentTxs.map(
                                (tx: any) => (
                                  <div
                                    key={tx.id}
                            ...
                            <p className="text-sm text-muted-foreground text-center py-8 border rounded-lg bg-muted/20 mt-3">
                    ...
                    },
                    {
                      ...
                      children: (
                        <StudentPerformanceTab
                          ...
                          loadingReportCard={reportCardModal.loading}
                        />
                      ),
                    },
                  ]}
        ...
        {reportCardModal.open && (
          <ReportCardModal
            isOpen={reportCardModal.open}
            onClose={() =>
              setReportCardModal({ open: false, loading: false, data: null })
            }
            reportCard={reportCardModal.data}
            zIndex={1200}
          />
        )}
