...
interface TodayRecord { classId: string; status: string; _count: { id: number }; }
...
const STATUS_CONFIG = {
  PRESENT: { label: "Present", color: "bg-green-500 text-white border-green-500", icon: CheckCircle },
  ABSENT: { label: "Absent", color: "bg-red-500 text-white border-red-500", icon: XCircle },
  LATE: { label: "Late", color: "bg-amber-500 text-white border-amber-500", icon: Clock },
...
export function AttendanceContent({
  ...
  const saveAttendance = async () => {
    ...
    const result = await markAttendance({ classId: selectedClassId, sectionId: selectedSectionId, date: selectedDate, records });
    setSaving(false);
  ...
  return (
    <div className="p-6 space-y-5 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Today's Rate", value: `${pct || "—"}%`, icon: Calendar, color: "bg-violet-100 text-violet-600 dark:bg-violet-900/30 dark:text-violet-400" },
          { label: "Present Today", value: (totalPresent || "—").toString(), icon: CheckCircle, color: "bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400" },
          { label: "Absent Today", value: (totalAbsent || "—").toString(), icon: XCircle, color: "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400" },
          { label: "Classes Today", value: classes.length.toString(), icon: Users, color: "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" },
        ].map((card) => (
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            >
              {loadingStudents ? <Loader2 className="w-3 h-3 animate-spin" /> : null}
              Load Students
            </button>
          </div>
        </div>
                      ...
                      {(Object.keys(STATUS_CONFIG) as AttendanceStatus[]).map((s) => (
                        <button
                          ...
                          className={`w-7 h-7 rounded-lg text-[9px] font-bold border-2 transition-all ${
                            status === s ? STATUS_CONFIG[s].color : "border-border text-muted-foreground hover:border-muted-foreground"
                          }`}
                        >
                          {s[0]}
                        </button>
        ...
        {students.length === 0 && !loadingStudents && (
          <div className="h-24 flex items-center justify-center text-xs text-muted-foreground">
            Select a class and section, then click "Load Students" to mark attendance
          </div>
        )}
      </div>
